...
function build_rootfs() {
	...
	if [ -f $rootfs_tarball ]; then
		...
		# Create build info file with build date and timestamp
		mkdir -p $rootfs_out_dir/etc
		cat > $rootfs_out_dir/etc/build-info <<EOF
Build Date: $BUILD_DATE
Build Timestamp: $BUILD_TIMESTAMP
Build Date (ISO): $(date -d @$BUILD_TIMESTAMP '+%Y-%m-%d %H:%M:%S' 2>/dev/null || date '+%Y-%m-%d %H:%M:%S')
EOF
		msg_info "Created build info: $rootfs_out_dir/etc/build-info"

		# Add overlay files
		if [ -d "${PROJECT_TOP_DIR}/overlay" ]; then
			msg_info "Copying overlay files..."
			rsync -a "${PROJECT_TOP_DIR}/overlay/" "${rootfs_out_dir}/"
		fi
	else
		msg_error "Not found rootfs tarball: $rootfs_tarball"
		exit 1

...
/*
 * SYNC Frame layout (v3, fixed 31 bytes):
 *  [0]      magic       0xBE
 *  [1]      version     0x02
 *  [2]      master_node_id
 *  [3]      sender_node_id
 *  [4..7]   superframe_seq   (LE32)
 *  [8..15]  master_time_us   (LE64, master CLOCK_REALTIME, log-only)
 ...
 *  [23]     relay_hops
 *  [24..27] slot_map[4]
 *  [28..29] bootstrap_window_us (LE16)
 *  [30]     bootstrap_period
 */
                     ...
                     uint8_t *out, size_t capacity, size_t *out_len)
{
	...
	write_le64(&out[8], in->master_time_us);
	...
	write_le16(&out[28], in->bootstrap_window_us);
	out[30] = in->bootstrap_period;
                       ...
                       sync_frame_t *out)
{
	...
	out->master_time_us = read_le64(&in[8]);
	...
	out->bootstrap_window_us = read_le16(&in[28]);
	out->bootstrap_period = in[30];
...
/*
 * DELAY_REQ layout (v2, fixed 8 bytes) — pure join/announce, no PTP timestamp:
 *  [0]      magic       0xBD
 *  [1]      version     0x02
 *  [2]      requester_node_id
 *  [3]      target_node_id

...
**G0 Status: READY FOR HARDWARE TEST** — весь код готов, smoke test scripts созданы.

- [x] **0.1** Обе стороны в monitor на одном канале: скрипт `scripts/unow-mon.sh`
  готов (поддерживает `iw dev <if> interface add`, fallback на `set monitor
  control otherbss`).
- [x] **0.2** `unow_diag` для LuckFox собирается без ошибок (ARM toolchain,
  uclibc-fixes, libpcap linked). RadioTap TX builder готов, self-dedup реализован.
- [ ] **0.3a** (АКТИВНО) Запустить smoke test на железе: LuckFox ↔ Ubuntu laptop
  с MW300UH адаптерами. Инструкции в [`TESTING.md`](./TESTING.md) и
  [`tests/README.md`](./tests/README.md).
- [ ] **0.3b** Замерить PER без потерь на 100 пакетов, зафиксить рабочий канал и
  legacy-rate (1–6 Мбит/с).
- [ ] **0.4** Запротоколировать результаты: лог `tcpdump`, PER%, RSSI min/max,
  latency. Создать баг-репорт если injection/capture не работает.
- [ ] **Артефакт G0:** Frames flowing bidirectionally, PER < 5%, RSSI readable.
  **Без прохода G0 проект 1 не стартует.**

**Инструменты:**
- `./media/unow/tests/smoke_test.sh` — автоматизированный RX/TX tester
- `UNOW_LOG_LEVEL={debug,trace}` — включить диагностику

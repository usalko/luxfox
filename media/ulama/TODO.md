...
### 3.1 L1 — формат кадра
- [ ] **3.1.1** Скопировать `ulama_frame.{c,h}` без изменений; собрать под arm/x86.
...
- [ ] Артефакт: `test_ulama_frame` зелёный на host и target; CRC-вектор совпал с ESP.
...
- [ ] **4.2** Backend **UDP** (MVP): `--listen 0.0.0.0:5000 --peer-port 5001`.
  ...
  лёгкого префикса (решить: маппинг `udp_port→{dst_node,class}`).
...
- [ ] Артефакт: `nc -u`/`socat` петля app↔ulamad↔(mock radio)↔ulamad↔app.
...
- [ ] Артефакт: `ulamad --role solo --iface mon0 --node 1` поднимается, логирует
  баннер, статистику раз в 1 с, корректно гасится по Ctrl-C.
...
- [ ] **6.1** Makefile: `CC := $(RK_MEDIA_CROSS)-gcc`, `-I include`,
  `-I src/compat/shim`, сборка `libulama.a` + бинаря `ulamad`. Линковка с
  `libunow` (проект 2), `-lpthread`, `-lm`.
- [ ] **6.2** Положить артефакты в `RK_MEDIA_OUTPUT` (правило
  `MAROC_COPY_PKG_TO_MEDIA_OUTPUT`, как в luxfox), бинарь → rootfs `/oem` или
  `/usr/bin`.
- [ ] `[!]` **6.3** Toolchain — **uclibc** (`arm-rockchip830-linux-uclibcgnueabihf`).
  Ядро ULAMA — чистый C, проблем не будет. Следить, чтобы compat-шим не тянул
  glibc-специфику.
- [ ] **6.4** Хост-сборка для тестов: тот же код собрать `gcc` под x86
  (Makefile-таргет `host`), чтобы юниттесты L1/L2/mesh гонять без железа.
- [ ] Артефакт: `./build.sh` (или прямой `make -C media`) собирает `ulamad`,
  он появляется в образе; `file ulamad` → ARM EABI uclibc.
...
media/ulama/
  ...
  src/
    ...
    main/                   # ulamad (§5)
  tests/                    # host-юниттесты L1/SLIP/mesh
  TODO.md                   # этот файл
```

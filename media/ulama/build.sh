...
stage_a_radio_only() {
    ...
    echo "  ✗ 'Base interface not found' error in unow-mon.sh:"
    echo "    → Check available ifaces: iw dev"
    echo "    → Use actual interface name (e.g. wlan0, not mon0)"
    echo "    → Correct: /oem/usr/bin/scripts/unow-mon.sh wlan0 mon0 6"
    echo "  ✗ 'ethtool package' warning:"
    echo "    → Not critical, continues anyway"

...
static bool usb2phy_reg_valid(const struct usb2phy_reg *reg)
{
	return reg->offset || reg->bitstart || reg->bitend ||
	       reg->disable || reg->enable;
}

static int usb2phy_reg_get_value(struct regmap *base,
				 const struct usb2phy_reg *reg)
{
	unsigned int orig;
	unsigned int mask;
	...
	mask = GENMASK(reg->bitend, reg->bitstart);
	return (orig & mask) >> reg->bitstart;
}

static const char *rockchip_usb2phy_mode_string(enum usb_dr_mode mode)
{
	switch (mode) {
...
static const char *rockchip_usb2phy_port_string(struct rockchip_usb2phy_port *rport)
{
	return rport->port_id == USB2PHY_PORT_OTG ? "otg" : "host";
}

static const char *rockchip_usb2phy_linestate_string(int utmi_ls)
{
	switch (utmi_ls) {
	case 0:
		return "SE0";
...
static void rockchip_usb2phy_port_log_state(struct rockchip_usb2phy_port *rport,
					    const char *reason)
{
	struct rockchip_usb2phy *rphy = dev_get_drvdata(rport->phy->dev.parent);
	const struct rockchip_usb2phy_port_cfg *cfg = rport->port_cfg;
	int utmi_avalid = usb2phy_reg_get_value(rphy->grf, &cfg->utmi_avalid);
	int utmi_bvalid = usb2phy_reg_get_value(rphy->grf, &cfg->utmi_bvalid);
	int utmi_iddig = usb2phy_reg_get_value(rphy->grf, &cfg->utmi_iddig);
	int utmi_ls = usb2phy_reg_get_value(rphy->grf, &cfg->utmi_ls);
	int utmi_hstdet = usb2phy_reg_get_value(rphy->grf, &cfg->utmi_hstdet);
	int utmi_vbus = rport->utmi_avalid ? utmi_avalid : utmi_bvalid;

	dev_info(&rport->phy->dev,
		 "%s: build=%s port=%s mode=%s susp=%d perip=%d host_disc=%d has_vbus=%d vbus_en=%d vbus_alw=%d vbus_att=%d id=%d avalid=%d bvalid=%d vbus=%d ls=%s(%d) hstdet=%d\n",
		 reason,
		 init_utsname()->release,
		 rockchip_usb2phy_port_string(rport),
		 rockchip_usb2phy_mode_string(rport->mode),
		 rport->suspended,
		 ...
		 utmi_bvalid,
		 utmi_vbus,
		 rockchip_usb2phy_linestate_string(utmi_ls),
		 utmi_ls,
		 utmi_hstdet);
}
...
static int rockchip_usb2phy_init(struct phy *phy)
{
	...
	rockchip_usb2phy_port_log_state(rport, "phy init");
				     ...
				     enum phy_mode mode, int submode)
{
	...
	rockchip_usb2phy_port_log_state(rport, "set_mode");
...
static irqreturn_t rockchip_usb2phy_linestate_irq(int irq, void *data)
{
	...
	rockchip_usb2phy_port_log_state(rport, "linestate irq");
...
static irqreturn_t rockchip_usb2phy_bvalid_irq(int irq, void *data)
{
	...
	rockchip_usb2phy_port_log_state(rport, "bvalid irq");
...
static irqreturn_t rockchip_usb2phy_id_irq(int irq, void *data)
{
	...
	rockchip_usb2phy_port_log_state(rport,
					cable_vbus_state ? "id irq host" : "id irq peripheral");
...
next_child:
	...
	dev_info(dev, "usb2phy debug ready: build=%s reg=0x%x ports=%u irq=%d\n",
		 init_utsname()->release, reg, rphy->phy_cfg->num_ports, rphy->irq);
...
static u32 rv1106_usb2phy_read_tuning_u32(struct rockchip_usb2phy *rphy,
						 const char *property,
						 u32 default_value,
						 u32 max_value)
{
	...
	if (of_property_read_u32(rphy->dev->of_node, property, &value))
		return default_value;

	if (value > max_value) {
		dev_warn(rphy->dev,
			 "%s=%u is out of range, clamping to %u\n",
			 property, value, max_value);
		value = max_value;
	}

	dev_info(rphy->dev, "rv1106 usb2phy override: %s=%u\n",
		 property, value);
...
{
	u32 tx_preemph_enable;
	u32 tx_preemph_strength;
	u32 rx_squelch_trigger;
	u32 hs_odt_value;
	u32 hs_eye_height;
	u32 hs_disconnect_mode;

	/* Always enable pre-emphasis in SOF & EOP & chirp & non-chirp state */
	tx_preemph_enable = rv1106_usb2phy_read_tuning_u32(rphy,
							   "rockchip,tx-preemphasis-enable-mask",
							   0x07, 0x07);
	phy_update_bits(rphy->phy_base + 0x30, GENMASK(2, 0), tx_preemph_enable);

	if (rockchip_get_cpu_version())
		/* Set Tx HS pre_emphasize strength to 3'b001 */
		tx_preemph_strength = 0x01;
	else
		/* Set Tx HS pre_emphasize strength to 3'b011 */
		tx_preemph_strength = 0x03;

	tx_preemph_strength = rv1106_usb2phy_read_tuning_u32(rphy,
							     "rockchip,tx-hs-preemphasis-strength",
							     tx_preemph_strength, 0x07);
	phy_update_bits(rphy->phy_base + 0x40, GENMASK(5, 3),
			tx_preemph_strength << 3);

	/* Set RX Squelch trigger point configure to 4'b0000(112.5 mV) */
	rx_squelch_trigger = rv1106_usb2phy_read_tuning_u32(rphy,
							   "rockchip,rx-squelch-trigger",
							   0x00, 0x0f);
	phy_update_bits(rphy->phy_base + 0x64, GENMASK(6, 3),
			rx_squelch_trigger << 3);

	/* Turn off differential receiver by default to save power */
	phy_clear_bits(rphy->phy_base + 0x100, BIT(6));

	/* Set 45ohm HS ODT value to 5'b10111 to increase driver strength */
	hs_odt_value = rv1106_usb2phy_read_tuning_u32(rphy,
						     "rockchip,tx-hs-odt-value",
						     0x17, 0x1f);
	phy_update_bits(rphy->phy_base + 0x11c, GENMASK(4, 0), hs_odt_value);

	/* Set Tx HS eye height tuning to 3'b011(462 mV) */
	hs_eye_height = rv1106_usb2phy_read_tuning_u32(rphy,
						      "rockchip,tx-hs-eye-height",
						      0x03, 0x07);
	phy_update_bits(rphy->phy_base + 0x124, GENMASK(4, 2),
			hs_eye_height << 2);
	...
	hs_disconnect_mode = rv1106_usb2phy_read_tuning_u32(rphy,
							     "rockchip,hs-disconnect-detect-mode",
							     0x01, 0x01);
	if (hs_disconnect_mode)
		phy_set_bits(rphy->phy_base + 0x70, BIT(2));
	else
		phy_clear_bits(rphy->phy_base + 0x70, BIT(2));

...
static void hub_port_log_status(struct usb_hub *hub, int port1,
				struct usb_port *port_dev,
				const char *reason, int status)
{
	...
	if (ret < 0) {
		dev_info(&port_dev->dev,
				 "%s: build=%s rc=%d failed to read port status=%d\n",
				 reason, init_utsname()->release, status, ret);
		return;
	}

	dev_info(&port_dev->dev,
		 "%s: build=%s rc=%d hcd=%s st=%04x ch=%04x conn=%d en=%d pwr=%d rst=%d susp=%d oc=%d cconn=%d cen=%d crst=%d coc=%d speed=%s\n",
		 reason, init_utsname()->release, status,
		 hcd->driver && hcd->driver->description ?
			 hcd->driver->description : dev_name(hcd->self.controller),
		 portstatus, portchange,
		 !!(portstatus & USB_PORT_STAT_CONNECTION),
		 !!(portstatus & USB_PORT_STAT_ENABLE),
		 hub_is_superspeed(hub->hdev) ?
...
static void hub_port_log_wait_reset_exit(struct usb_hub *hub, int port1,
					 u16 portstatus, u16 portchange,
					 int status, const char *why)
{
	struct usb_port *port_dev = hub->ports[port1 - 1];

	dev_info(&port_dev->dev,
		 "wait_reset exit: build=%s rc=%d why=%s st=%04x ch=%04x conn=%d en=%d pwr=%d rst=%d susp=%d oc=%d cconn=%d cen=%d crst=%d coc=%d speed=%s\n",
			...
			struct usb_device *udev, unsigned int delay, bool warm)
{
	...
	if ((portstatus & USB_PORT_STAT_RESET)) {
		hub_port_log_wait_reset_exit(hub, port1, portstatus, portchange,
					   -EBUSY,
					   "port_still_in_reset");
		return -EBUSY;
	}
	...
	if (!hub_is_superspeed(hub->hdev) &&
		...
		hub_port_log_wait_reset_exit(hub, port1, portstatus, portchange,
					   -EAGAIN,
					   "connect_change_while_connected");
		return -EAGAIN;
	}
		...
		if (status == -ENODEV) {
			...
			if (status && status != -ENOTCONN && status != -ENODEV) {
				dev_dbg(hub->intfdev,
						...
						status);
				hub_port_log_status(hub, port1, port_dev,
						warm ? "warm reset wait failed" :
						"reset wait failed",
						status);
			}
		}
